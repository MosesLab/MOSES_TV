...
if n_elements(sizes) ne 4 then sizes = replicate(2097152L,3)
;if sizes[0] ne 0 then n_read = uintarr(sizes[0])
m_read = uintarr(sizes[0])
z_read = uintarr(sizes[1])
p_read = uintarr(sizes[2])
...
if error eq 0 then begin
   ;print, "debugging check 1: noise = "+string(sizes[0])
   ;print, "debugging check 2: plus = "+string(sizes[3])
   ;print, "debugging check 3: array_size = "+string(n_elements(sizes))
   ;if sizes[0] ne 0 then readu,1,n_read,m_read,z_read,p_read $
   readu,1, m_read,z_read,p_read
   close,1
   ;Now reformat the data into image arrays
   n = uintarr(2048,1024) & m=n & z=n & p=n
   ;if sizes[0] ne 0 then n[0:sizes[0]-1] = n_read
   m[0:sizes[0]-1] = m_read
   z[0:sizes[1]-1] = z_read
   p[0:sizes[2]-1] = p_read
